-- Supabase helper script for onboarding.html
-- 1. Drops and recreates the `profiles` table with every identity/compliance column
--    referenced by the onboarding flow.
-- 2. Mirrors the field names used inside `public/onboarding.html` so you can copy
--    this definition straight into the SQL editor.

-- WARNING: dropping the table deletes all profile data. Remove the DROP statement
-- if you only need the CREATE TABLE definition.
